...
TEST_EXE_PATH=/Users/manfredo/Documents/Eclipse_workspace/ED/build/ed_2.1-opt
DBUG_EXE_PATH=/Users/manfredo/Documents/Eclipse_workspace/ED/build/ed_2.1-dbg
MAIN_EXE_PATH=/Users/manfredo/Documents/Eclipse_workspace/ED/build/ed_2.1-opt

# Provide the path where the test-suit driver archive is stored

DATAPATH='/Users/manfredo/Documents/Eclipse_workspace/ED/build'
...
declare -a USE_SITE=( \
    ...
    $USE_TNF $USE_ATA $USE_GYF)
declare -a SITEID=(m34 s67 har pdg ton cax tnf ata gyf)
declare -a SITEPFX=(M34 S67 HAR PDG TON CAX TNF ATA GYF)
declare -a SITEQ=( \
    $Q_M34 $Q_S67 $Q_HAR \
    $Q_PDG $Q_TON $Q_CAX \
    $Q_TNF $Q_ATA $Q_GYF)
...
# ========================
  ...
  gsed -i '/NL%RUNTYPE/c\   NL%RUNTYPE = '\'${RUNTYPS[i]}\' $FILEMAIN
  gsed -i '/NL%RUNTYPE/c\   NL%RUNTYPE = '\'${RUNTYPS[i]}\' $FILETEST
  gsed -i '/NL%RUNTYPE/c\   NL%RUNTYPE = '\'${D_RUNTYPS[i]}\' $FILEDBUG

# Modify the ED2IN IED_INIT_MODE
# ==============================

  gsed -i '/NL%IED_INIT_MODE/c\   NL%IED_INIT_MODE = '${INITMDS[i]} $FILEMAIN
  gsed -i '/NL%IED_INIT_MODE/c\   NL%IED_INIT_MODE = '${INITMDS[i]} $FILETEST
  gsed -i '/NL%IED_INIT_MODE/c\   NL%IED_INIT_MODE = '${D_INITMDS[i]} $FILEDBUG

# Modify the ED2IN start years
# ============================

  gsed -i '/NL%IYEARA/c\   NL%IYEARA   = '${IYEARAS[i]} $FILEMAIN
  gsed -i '/NL%IYEARA/c\   NL%IYEARA   = '${IYEARAS[i]} $FILETEST
  gsed -i '/NL%IYEARA/c\   NL%IYEARA   = '${D_IYEARAS[i]} $FILEDBUG

# Modify the ED2IN end years
# ==========================

  gsed -i '/NL%IYEARZ/c\   NL%IYEARZ   = '${IYEARZS[i]} $FILEMAIN
  gsed -i '/NL%IYEARZ/c\   NL%IYEARZ   = '${IYEARZS[i]} $FILETEST
  gsed -i '/NL%IYEARZ/c\   NL%IYEARZ   = '${D_IYEARZS[i]} $FILEDBUG

# Modify the ED2IN files to point to the desired output directories   
# =================================================================   
  
  gsed -i '/NL%FFILOUT/c\   NL%FFILOUT = '\''F_main_'${SITEID[i]}'/main_'${SITEID[i]}''\' $FILEMAIN
  gsed -i '/NL%SFILOUT/c\   NL%SFILOUT = '\''S_main_'${SITEID[i]}'/main_'${SITEID[i]}''\' $FILEMAIN
  gsed -i '/NL%FFILOUT/c\   NL%FFILOUT = '\''F_test_'${SITEID[i]}'/test_'${SITEID[i]}''\' $FILETEST
  gsed -i '/NL%SFILOUT/c\   NL%SFILOUT = '\''S_test_'${SITEID[i]}'/test_'${SITEID[i]}''\' $FILETEST
  gsed -i '/NL%FFILOUT/c\   NL%FFILOUT = '\''F_dbug_'${SITEID[i]}'/dbug_'${SITEID[i]}''\' $FILEDBUG
  gsed -i '/NL%SFILOUT/c\   NL%SFILOUT = '\''S_dbug_'${SITEID[i]}'/dbug_'${SITEID[i]}''\' $FILEDBUG

# Modify the state file output frequency
# ======================================

  gsed -i '/NL%UNITSTATE/c\   NL%UNITSTATE  = '$UNITSTATE $FILEMAIN
  gsed -i '/NL%UNITSTATE/c\   NL%UNITSTATE  = '$UNITSTATE $FILETEST
  gsed -i '/NL%UNITSTATE/c\   NL%UNITSTATE  = '$UNITSTATE $FILEDBUG
...
# Modify the ED2IN RUNTYPE
# ========================
  gsed -i '/NL%RUNTYPE/c\   NL%RUNTYPE = '\'${RUNTYPH[i]}\' $FILEMAIN
  gsed -i '/NL%RUNTYPE/c\   NL%RUNTYPE = '\'${RUNTYPH[i]}\' $FILETEST
  gsed -i '/NL%RUNTYPE/c\   NL%RUNTYPE = '\'${RUNTYPH[i]}\' $FILEDBUG

# Modify the ED2IN IED_INIT_MODE
# ==============================

  gsed -i '/NL%IED_INIT_MODE/c\   NL%IED_INIT_MODE = '${INITMDH[i]} $FILEMAIN
  gsed -i '/NL%IED_INIT_MODE/c\   NL%IED_INIT_MODE = '${INITMDH[i]} $FILETEST
  gsed -i '/NL%IED_INIT_MODE/c\   NL%IED_INIT_MODE = '${INITMDH[i]} $FILEDBUG


# Modify the ED2IN start dates
# ============================

  gsed -i '/NL%IDATEA/c\   NL%IDATEA   = '${IDATEAH[i]} $FILEMAIN
  gsed -i '/NL%IDATEA/c\   NL%IDATEA   = '${IDATEAH[i]} $FILETEST
  gsed -i '/NL%IDATEA/c\   NL%IDATEA   = '${IDATEAH[i]} $FILEDBUG

# Modify the ED2IN end date
# ==========================

  gsed -i '/NL%IDATEZ/c\   NL%IDATEZ   = '${IDATEZH[i]} $FILEMAIN
  gsed -i '/NL%IDATEZ/c\   NL%IDATEZ   = '${IDATEZH[i]} $FILETEST
  gsed -i '/NL%IDATEZ/c\   NL%IDATEZ   = '${IDATEZH[i]} $FILEDBUG

# Modify the ED2IN files to point to the desired output directories   
# =================================================================   
  
  gsed -i '/NL%FFILOUT/c\   NL%FFILOUT = '\''F_main_'${HIFRID[i]}'/main_'${HIFRID[i]}''\' $FILEMAIN
  gsed -i '/NL%SFILOUT/c\   NL%SFILOUT = '\''S_main_'${HIFRID[i]}'/main_'${HIFRID[i]}''\' $FILEMAIN
  gsed -i '/NL%FFILOUT/c\   NL%FFILOUT = '\''F_test_'${HIFRID[i]}'/test_'${HIFRID[i]}''\' $FILETEST
  gsed -i '/NL%SFILOUT/c\   NL%SFILOUT = '\''S_test_'${HIFRID[i]}'/test_'${HIFRID[i]}''\' $FILETEST
  gsed -i '/NL%FFILOUT/c\   NL%FFILOUT = '\''F_dbug_'${HIFRID[i]}'/dbug_'${HIFRID[i]}''\' $FILEDBUG
  gsed -i '/NL%SFILOUT/c\   NL%SFILOUT = '\''S_dbug_'${HIFRID[i]}'/dbug_'${HIFRID[i]}''\' $FILEDBUG
...
# ========================
  ...
  gsed -i '/NL%RUNTYPE/c\   NL%RUNTYPE = '\'${RUNTYPG[i]}\' $FILEMAIN
  gsed -i '/NL%RUNTYPE/c\   NL%RUNTYPE = '\'${RUNTYPG[i]}\' $FILETEST
  gsed -i '/NL%RUNTYPE/c\   NL%RUNTYPE = '\'${D_RUNTYPG[i]}\' $FILEDBUG

# Modify the ED2IN IED_INIT_MODE
# ==============================

  gsed -i '/NL%IED_INIT_MODE/c\   NL%IED_INIT_MODE = '${INITMDG[i]} $FILEMAIN
  gsed -i '/NL%IED_INIT_MODE/c\   NL%IED_INIT_MODE = '${INITMDG[i]} $FILETEST
  gsed -i '/NL%IED_INIT_MODE/c\   NL%IED_INIT_MODE = '${D_INITMDG[i]} $FILEDBUG

# Modify the ED2IN start years
# ============================

  gsed -i '/NL%IYEARA/c\   NL%IYEARA   = '${IYEARAG[i]} $FILEMAIN
  gsed -i '/NL%IYEARA/c\   NL%IYEARA   = '${IYEARAG[i]} $FILETEST
  gsed -i '/NL%IYEARA/c\   NL%IYEARA   = '${D_IYEARAG[i]} $FILEDBUG

# Modify the ED2IN end years
# ==========================

  gsed -i '/NL%IYEARZ/c\   NL%IYEARZ   = '${IYEARZG[i]} $FILEMAIN
  gsed -i '/NL%IYEARZ/c\   NL%IYEARZ   = '${IYEARZG[i]} $FILETEST
  gsed -i '/NL%IYEARZ/c\   NL%IYEARZ   = '${D_IYEARZG[i]} $FILEDBUG

# Modify the ED2IN files to point to the desired output directories
# =================================================================

  gsed -i '/NL%FFILOUT/c\   NL%FFILOUT = '\''F_main_'${GRIDID[i]}'/main_'${GRIDID[i]}''\' $FILEMAIN
  gsed -i '/NL%SFILOUT/c\   NL%SFILOUT = '\''S_main_'${GRIDID[i]}'/main_'${GRIDID[i]}''\' $FILEMAIN
  gsed -i '/NL%FFILOUT/c\   NL%FFILOUT = '\''F_test_'${GRIDID[i]}'/test_'${GRIDID[i]}''\' $FILETEST
  gsed -i '/NL%SFILOUT/c\   NL%SFILOUT = '\''S_test_'${GRIDID[i]}'/test_'${GRIDID[i]}''\' $FILETEST
  gsed -i '/NL%FFILOUT/c\   NL%FFILOUT = '\''F_dbug_'${GRIDID[i]}'/dbug_'${GRIDID[i]}''\' $FILEDBUG
  gsed -i '/NL%SFILOUT/c\   NL%SFILOUT = '\''S_dbug_'${GRIDID[i]}'/dbug_'${GRIDID[i]}''\' $FILEDBUG
